 ...
 * pageview and every custom event, and returning `null` drops it. It is read at send time rather
 * than at mount, so it holds for a switch flipped in the middle of a session.
 *
 ...
 * the script - and the `window.va` queue it installed - for the rest of the document.
 *
 ...
 */
function keepVercelBeaconSilentWhileCollectionIsOff(
  event: BeforeSendEvent,
): BeforeSendEvent | null {
  return isSiteAnalyticsCollectionEnabled() ? event : null;
}
 ...
 * therefore loads no vendor script at all unless it turns collection back on.
 *
 ...
 * visitor never navigated to again. The mount gate therefore protects exactly one browser: the one
 * that arrives with collection already off.
 */
export function SiteVercelAnalytics(): React.JSX.Element | null {
  ...
  return isCollectionEnabled || hasMountedVercelBeacon ? (
    <Analytics beforeSend={keepVercelBeaconSilentWhileCollectionIsOff} />
  ) : null;
}
