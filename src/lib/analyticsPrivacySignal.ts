 ...
 * and never reports a click to our own collector, so the suppression sits ahead of both the identity
 * request and the collector rather than only in front of the collector: asking
 * `GET /v1/analytics/visitor` outside a consent jurisdiction mints a 13-month cookie, which is
 * precisely what the signal opts out of.
 */
