...
4. Nibomo MCP सर्वर और बैकएंड

कुछ उत्पादों में क्लाइंट और मॉडल प्रदाता की भूमिका एक ही सेवा निभाती है। दूसरे उत्पाद टूल के नतीजे किसी अलग सेवा को भेजते हैं। एक बात तय है: Nibomo माँगा गया डेटा प्रमाणित MCP क्लाइंट को लौटाता है। उसके बाद क्या होता है, यह क्लाइंट की बनावट, प्लान और सेटिंग्स पर निर्भर करता है। नतीजा मॉडल के context में जा सकता है, एक ही प्रदाता के infrastructure में रह सकता है, या किसी दूसरे processor तक पहुँच सकता है।
...
## Nibomo OAuth क्या सुरक्षित करता है

Nibomo interactive MCP clients के लिए PKCE और Dynamic Client Registration वाला authorization-code flow इस्तेमाल करता है। आप browser में connection approve करते हैं, और PKCE code exchange को उसी client से बाँधता है जिसने उसे शुरू किया था। Server यह भी जाँचता है कि access token Nibomo MCP resource के लिए ही जारी किया गया था। [Nibomo MCP connector guide](/hi/docs/mcp-connector/) में endpoint और discovery metadata दिए गए हैं।
...
## Nibomo के MCP टूल असल में क्या कर सकते हैं
...
हर SQL call ठीक एक workspace पर चलती है। Statement चलने से पहले Nibomo फिर जाँचता है कि उपयोगकर्ता के पास अब भी उस workspace की पहुँच है। इससे call करने वाला व्यक्ति किसी और के workspace की ID देकर वहाँ नहीं पहुँच सकता।
...
लौटाया गया डेटा फिर भी Nibomo backend से बाहर जाता है। कमजोर विषयों की query में कार्ड का टेक्स्ट और review events शामिल हो सकते हैं। किसी छोटे कार्ड में भी मरीज की जानकारी, किसी internal system का नाम, भाषा सीखने का निजी उदाहरण या interview notes हो सकते हैं।

[Nibomo privacy policy](/hi/privacy/) MCP और Agent API से माँगे गए डेटा पर लागू होती है। Protocol की सीमा पर Nibomo नतीजा MCP client को भेजता है। कोई अलग model provider उसे पाएगा या नहीं, कोई उसे कितने समय तक रखेगा, और training के लिए उसका इस्तेमाल हो सकता है या नहीं—यह client setup और provider की शर्तों पर निर्भर करता है। "Read-only" को privacy की मुहर मानने के बजाय ये बातें जाँचें।
...
[सेवा की शर्तें](/hi/terms/) उपयोगकर्ताओं से बदलाव लागू करने से पहले AI से मिला output review करने को कहती हैं। यहाँ यह खास तौर पर ज़रूरी है: Nibomo statement family, resource, columns और row cap validate कर सकता है। वह यह नहीं परख सकता कि आपने बदलाव किस वजह से माँगा था।

## मंज़ूरी AI client की ज़िम्मेदारी है

Nibomo `sql_query` और बाकी read tools को `readOnlyHint` से, और `sql_execute` तथा `submit_review` को `destructiveHint` से mark करता है। [25 नवंबर 2025 की स्थिर MCP schema](https://modelcontextprotocol.io/specification/2025-11-25/schema) में tool annotations साफ़ तौर पर hints हैं। वे compatible client को approval policy चुनने में मदद करते हैं; खुद उस policy को लागू नहीं करते।

Nibomo को valid और authenticated `sql_execute` या `submit_review` call मिलते ही वह उसे तुरंत चला देता है। Nibomo में दूसरी confirmation screen नहीं आती। इंसानी मंज़ूरी के लिए कोई भी pause, request के server तक पहुँचने से पहले AI client में होता है।
...
Hosted account delete करना, connector disconnect करना और आगे बनी copies delete करना अलग-अलग काम हैं। [Nibomo privacy](/hi/privacy/) hosted data delete करने का तरीका बताती है। Client या model provider के पास पहले से रखा data Nibomo delete नहीं कर सकता; उसके लिए उस provider के controls भी इस्तेमाल करें।

## Disconnect करना और credential revoke करना अलग कदम हैं

Nibomo का मौजूदा OAuth implementation एक घंटे के access tokens और बिना तय expiry वाले rotating refresh tokens जारी करता है। Nibomo अभी users के लिए OAuth connection revoke करने वाली UI या public revocation endpoint नहीं देता। Connector हटाने पर client अपने credentials discard कर सकता है। Client की यह कार्रवाई server-side token invalidation की गारंटी नहीं देती।

Headless `fca_` Agent API keys अलग तरह के credentials हैं। Nibomo में **Agent Connections** के जरिए इन keys को revoke किया जा सकता है। पहुँच को दर्ज या बंद करते समय authentication के दोनों रास्तों को अलग रखें।
...
इन फैसलों के बाद setup steps के लिए [MCP के साथ Nibomo को Claude से कैसे जोड़ें](/hi/blog/how-to-connect-flashcards-to-claude-with-mcp/) देखें। Connection guide हर click समझाती है; यह checklist तय करती है कि किसी खास deck के पास connection होना भी चाहिए या नहीं।

## Open source और self-hosting कहाँ मदद करते हैं

Nibomo connector में कई उपयोगी खूबियाँ हैं: अलग read और write tools, statements की सीमित allowlist, हर call पर workspace membership checks, ऐसे scheduling fields जिन्हें SQL नहीं लिख सकता, और public source code। इन controls से connector के दायरे की जाँच करना और उसे सीमित रखना आसान होता है। वे जोखिम घटाते हैं; सुरक्षित client या model के सही फैसले की गारंटी नहीं देते।

[Self-hosted deployment](/hi/docs/self-hosting/) Nibomo storage और operations को ऐसे infrastructure पर ला सकता है जिसे आप नियंत्रित करते हैं। External AI service को भेजी गई queries card data को फिर भी उस deployment से बाहर ले जाती हैं। Model और client वाला रास्ता भी उसी privacy standard पर खरा उतरना चाहिए जो database के लिए है।

## फैसला करने का एक सरल नियम

MCP read tools तभी इस्तेमाल करें, जब माँगा गया data चुने हुए client path के जरिए Nibomo से बाहर जा सकता हो और यह आपको स्वीकार हो, provider की terms ठीक हों, और task उस data को साझा करना उचित ठहराता हो। जब तक client ने सच में `sql_execute` और `submit_review` block न किए हों, connection को full-access मानें।
...
जब deck को client या उसके processors के साथ share नहीं किया जा सकता, downstream policy साफ़ नहीं है, उसी account में सख़्त workspace isolation चाहिए, तुरंत OAuth revocation अनिवार्य है, या workflow में बिना निगरानी के destructive writes चलाने हैं, तो connection छोड़ दें। ऐसे मामलों में MCP के बिना Nibomo इस्तेमाल करें, या ऐसा deployment और model path चुनें जिसका पूरा data flow आपकी requirements पूरी करता हो।
