...
- [External Agent API](/docs/api/) `GET https://api.nibomo.com/v1/` से शुरू होता है। उसका discovery response agent को email OTP login, API-key creation और workspace selection की प्रक्रिया बताता है। पढ़ने के लिए SQL-style query route है और लिखने के लिए अलग execute route।
- [Remote MCP server](/docs/mcp-connector/) `https://mcp.nibomo.com/mcp` पर उपलब्ध है। MCP clients को सात tools मिलते हैं: `list_workspaces`, `sql_query`, `sql_execute`, `get_guide` और review tools `next_review_card`, `reveal_answer` तथा `submit_review`।
