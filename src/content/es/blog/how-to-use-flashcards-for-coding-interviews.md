...
La semana pasada resolví un problema de grafos, me sentí peligrosamente competente durante un rato y luego olvidé la forma exacta en que había fallado mi primer intento apenas dos días después. Ahí es cuando **las flashcards para entrevistas de programación** empiezan a sonar menos a cosa de nerds y más a algo práctico.

No porque las entrevistas de programación sean solo memorizar.

No lo son.

Pero sí exigen recordar muchas cosas bajo presión:
...
Por eso tiene sentido usar **repetición espaciada para preparar entrevistas de programación**. La meta no es memorizar soluciones completas como si fueras un actor en escena. La meta es que las partes útiles te salgan con más facilidad cuando el cronómetro ya está corriendo.
...
- qué hace fallar una actualización de límites en binary search
...
Simplemente no puedes recuperarlo con suficiente rapidez.
...
Ya existen herramientas dedicadas construidas alrededor de **flashcards de LeetCode** y la repetición espaciada, y las plataformas de estudio generalistas siguen impulsando mazos de informática, formatos tipo quiz y material de estudio generado con IA en lugar de limitarse a lecturas estáticas. Eso sugiere que la necesidad ya no es teórica.
...
"¿Qué de este problema merece que lo recuerde rápido la próxima vez que vea algo parecido?"
...
Si fallas, te quedas sin tiempo o eliges primero el camino equivocado, acabas de encontrar material excelente para flashcards.
...
- un tradeoff de system design que sigues explicando de manera imprecisa
...
¿Cuándo debería considerar una sliding window como una de las primeras opciones?
...
¿Qué invariante mantiene válida la detección de ciclos con puntero rápido y puntero lento?
...
Solo son útiles cuando la estructura se repite lo suficiente.
...
## Algoritmos, system design y curiosidades del lenguaje no deberían compartir un único mazo sin plan
...
Así no estás creando un mazo nuevo cada vez que el proceso de una empresa se vuelve especialmente intenso.
...
Eso normalmente se repasa mal.

Yo mantendría el frente bien acotado.
...
Si necesitas el contexto extra, ponlo en el reverso.

Si necesitas tres objetivos de recuperación distintos, crea tres tarjetas.

La entrevista no te va a pedir que recuerdes un artículo entero en una sola respiración.
...
Muchos desarrolladores ya están usando IA para explicar problemas, comparar soluciones y generar implementaciones alternativas. Eso hace mucho más fácil producir posibles tarjetas a partir de:
...
Eso basta para una **preparación seria para entrevistas de software engineering**.
...
- archivos adjuntos cuando quieres pegar notas, capturas o material exportado dentro del flujo de trabajo con IA
...
- alojamiento de código abierto si quieres que todo tu sistema de preparación sea inspeccionable y esté bajo tu control

Esa combinación importa porque las **flashcards de algoritmos** solo funcionan si el flujo de repaso diario sigue siendo ligero. Si la herramienta hace molesto capturar o recuperar información, volverás en silencio a leer las mismas explicaciones y a llamarlo repaso.
...
No uses flashcards para memorizar una entrevista de programación de principio a fin.
