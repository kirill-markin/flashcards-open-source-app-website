...
另外，目录安装链接会向我们自己的产品分析收集端发送点击事件，包含公开软件包版本标识符、页面界面语言、浏览器提供的设备语言、链接位置、来源类别和设备类别。此收集端不会根据点击推断国家。如果浏览器发出 Global Privacy Control 或 Do Not Track 信号，这些网站点击事件便不会发送。

托管网页应用使用 `otp_session`、`session`、`refresh` 和 `logged_in` 等严格必要的 Cookie，以完成身份验证、维持会话、刷新访问权限并显示登录状态。禁用这些 Cookie 后，浏览器登录流程将无法工作。

托管网页应用还会在产品域名上设置一个我们自己的分析 Cookie `analytics_visitor`。它保存一个随机的访客标识符，可由我们在浏览器中的脚本读取，13个月后过期。我们用它在多次访问之间识别同一浏览器，使产品使用事件体现为回访访客，而不是每次都算作新访客。该 Cookie 并非严格必要。在需要征得同意的地区，包括欧盟/欧洲经济区和英国，我们只在你同意后才设置，你之后可以在网页应用的设置中撤回同意，撤回后不再使用该 Cookie。事件本身只在本政策所述目的仍有需要时保留。
