...
另外，目录安装链接会向我们自己的产品分析收集端发送点击事件，包含公开软件包版本标识符、页面界面语言、浏览器提供的设备语言、链接位置、来源类别、设备类别，以及在你的浏览器持有时下文所述的 `analytics_visitor` 标识符。此收集端不会根据点击推断国家。如果浏览器发出 Global Privacy Control 或 Do Not Track 信号，网站不会向此收集端发送点击事件，不会请求访客标识符，也不会显示同意横幅。

托管网页应用使用 `otp_session`、`session`、`refresh` 和 `logged_in` 等严格必要的 Cookie，以完成身份验证、维持会话、刷新访问权限并显示登录状态。禁用这些 Cookie 后，浏览器登录流程将无法工作。

营销网站和托管网页应用共用一个我们自己的分析 Cookie `analytics_visitor`。它保存一个随机的访客标识符，可由我们在浏览器中的脚本读取，13个月后过期。我们用它在多次访问之间识别同一浏览器，使事件体现为回访访客，而不是每次都算作新访客。该 Cookie 并非严格必要。在需要征得同意的地区，包括欧盟/欧洲经济区和英国，我们只在你同意后才设置：网站会以横幅询问，在你作出回答之前不会请求标识符，也不会附加标识符；上文所述的目录安装链接点击事件在此期间仍会发送，只是不带标识符。在无需同意的地区，我们不询问即设置。无论哪种情况，你都可以随时通过网站每个页面角落的分析控件或网页应用的设置将其关闭；之后我们会从你的浏览器中清除该 Cookie 并停止使用。你的回答会保存在你的浏览器中，以免重复询问。事件本身只在本政策所述目的仍有需要时保留。
