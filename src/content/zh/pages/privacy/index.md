...
网站每个页面角落的分析控件包含关闭本网站收集的开关，以及与之并列的上文所述分析 Cookie 同意；在你回应横幅之后，收集处于开启状态时控件也会提供该 Cookie 选择，你重新开启收集后，它也会再次出现。关闭时会发送最后一个事件，仅记录它被关闭这件事；重新开启时会发送与之对应的事件。两者都不带标识符、不带每日标识符，也不带国家。收集关闭期间，网站不会再发送任何关于你本次访问的其他事件，既不发送给我们的收集端，也不发送给 Vercel Web Analytics，并且不会请求访客标识符；由于此外没有任何数据到达我们的收集端，也不会生成每日标识符。除非你将其关闭，它默认开启，依据“处理目的与法律依据”部分所述的正当利益。本网站没有账户，因此你的回答保存在该浏览器中，并且仅在该浏览器生效；托管网页应用有自己的设置，iOS 和 Android 应用则从引入该设置的应用版本起具备，二者详见“产品分析”部分。
