...
营销网站和托管网页应用共用一个我们自己的分析 Cookie `analytics_visitor`。它保存一个随机的访客标识符，可由我们在浏览器中的脚本读取，13个月后过期。我们用它在多次访问之间识别同一浏览器，使事件体现为回访访客，而不是每次都算作新访客。该 Cookie 并非严格必要。这一选择只决定你的浏览器是否携带该标识符；网站是否收集任何信息，由下文所述的另一个开关决定。在需要征得同意的地区，包括欧盟/欧洲经济区和英国，我们只在你同意后才设置：网站会以横幅询问，在你作出回答之前不会请求标识符，也不会附加标识符；上文所述的页面浏览事件和点击事件在此期间仍会发送，只是不带该标识符，仅会获得下一段所述的每日标识符。在无需同意的地区，我们不询问即设置。无论哪种情况，你都可以随时通过网站每个页面角落的分析控件或网页应用的设置将其关闭；之后我们会从你的浏览器中清除该 Cookie 并停止使用。你的回答会保存在你的浏览器中，以免重复询问。事件本身只在本政策所述目的仍有需要时保留。

当页面浏览事件或点击事件在不带 `analytics_visitor` 标识符的情况下到达我们的收集端时（例如你拒绝了该 Cookie、尚未作出回答或已将其关闭），我们的服务器会根据该请求的 IP 地址和浏览器用户代理，结合一个每个 UTC 日更换、并在当日结束后删除的随机值，生成一个临时标识符。IP 地址本身不会保存在我们的分析数据中。该标识符只会关联同一天内来自同一浏览器和同一 IP 地址的页面浏览和点击。它从不保存在你的浏览器中，从不与 `analytics_visitor` 标识符或任何账户关联，从不附加到你的同意回答记录上，并且在当天的随机值删除后无法再被重新计算。我们使用它来了解访客如何使用网站，依据是“处理目的与法律依据”一节所述的我们的合法利益。如果你的浏览器发出 Global Privacy Control 或 Do Not Track 信号，网站不会向此收集端发送任何内容，因此也不会生成此类标识符。

网站每个页面角落的分析控件包含关闭本网站收集的开关，以及与之并列的上文所述分析 Cookie 同意；在你回应横幅之后，收集处于开启状态时控件也会提供该 Cookie 选择，你重新开启收集后，它也会再次出现。收集关闭期间，网站不会发送任何关于你本次访问的页面浏览事件或点击事件，既不发送给我们的收集端，也不发送给 Vercel Web Analytics，并且不会请求访客标识符；由于没有任何数据到达我们的收集端，也不会生成每日标识符。除非你将其关闭，它默认开启，依据“处理目的与法律依据”部分所述的正当利益。本网站没有账户，因此你的回答保存在该浏览器中，并且仅在该浏览器生效；托管网页应用有自己的设置，iOS 和 Android 应用则从引入该设置的应用版本起具备，二者详见“产品分析”部分。
