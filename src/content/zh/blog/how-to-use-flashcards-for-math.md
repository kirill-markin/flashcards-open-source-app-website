...
如果你想看一个具体而且刻意收窄范围的例子，可以用[单元圆闪卡：16 个角度、弧度与精确坐标](/catalog/packages/unit-circle-degrees-radians-coordinates/)练习单元圆上的 16 个标准位置，并在角度、弧度和精确坐标之间建立对应，而不用把整个主题塞进一张信息过载的卡片。

另一个更贴近数学方法选择的例子是这套[独立编写的高考数学导数闪卡](/catalog/packages/gaokao-derivatives-flashcards-chinese/)：76 张中文卡把求导条件、切线、符号表、单调性、极值和含参题入口拆成单一回忆目标，不收录高考题原文。
