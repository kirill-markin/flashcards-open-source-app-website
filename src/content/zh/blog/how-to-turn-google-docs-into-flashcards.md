...
只要你把相关文字从文档里提出来，Nibomo 就已经能发挥作用。当前产品提供 AI chat、文件附件、纯文本输入、正反面卡片创建，以及 FSRS 复习。它并没有承诺什么特殊的 Google Docs 集成，不会自动读取一份正在协作的共享文档，然后悄悄替你变成一套干净卡组。这反而是更诚实的工作流。你可以在 [功能页](/features/) 看到当前产品的功能范围。
...
4. 把清理后的这部分复制到 Nibomo 的 AI chat 里；如果那样更方便检查，就附上导出的文件。
...
## 复制或导出之后，Nibomo 真正接手的部分

一旦 Google Doc 里真正有用的那部分被整理出来，Nibomo 接手的就是最关键的阶段：
