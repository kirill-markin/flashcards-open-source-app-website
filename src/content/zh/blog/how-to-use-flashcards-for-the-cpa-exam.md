---
title: "2026 年如何用 Flashcards 备考 CPA 考试：真正能记住的 FAR、AUD、REG 和 Discipline 主题"
description: "准备参加 2026 年 CPA 考试？这是一套实用的 Flashcards 工作流，帮你整理 FAR 公式、AUD 认定、REG 规则、Discipline 科目重点，以及 task-based simulations 错题，并结合 AI 起草与 FSRS 复习。"
date: "2026-04-28"
...
![在暖色学习笔记和 task-based simulation 改题笔记旁复习 FAR、AUD 和 REG 抽认卡的 CPA 考生](/blog/how-to-use-flashcards-for-the-cpa-exam.png)
...
- `fact-gap` 卡
- `distinction` 卡
- `threshold` 或 `formula` 卡
- 来自 simulation 错题的 `process` 卡

我会把每周循环保持得很简单：

1. 做一组聚焦明确的练习题，或者一个 simulation block。
2. 只标记那些看起来值得复用的错题。
3. 把解析、你的笔记，或相关摘录贴进 AI chat。
4. 让它先给出少量正反面候选卡，而不是一整副巨型牌组。
5. 含糊卡片立刻删除。
6. 给留下来的卡片按分部和功能打标签，比如 `far`、`aud`、`reg`、`bar`、`isc`、`tcp`、`formula`、`assertion`、`tbs-miss` 或 `threshold`。
...
1. 先把卡片写小
2. 让 FAR、AUD、REG、Discipline 和 simulation 错题卡彼此分开
3. 尽早删除含糊的卡
4. 停止导入整段解析
5. 等牌组值得被排程之后，再让 FSRS 接手
