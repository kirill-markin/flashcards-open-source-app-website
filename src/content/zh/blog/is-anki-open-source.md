...
Flashcards 提供了完整的 [AWS 全栈自托管文档](/zh/docs/self-hosting/)，但它并不是一条命令就能完成的通用部署方案。运营者需要负责 AWS、Cloudflare、身份验证、电子邮件发送、数据库恢复、监控和升级，还要单独构建与分发原生应用。[Flashcards 自托管指南](/zh/blog/self-hosted-open-source-flashcards-app-for-spaced-repetition/)提供了更简短的产品视角，[Anki 与 Nibomo 对比](/zh/blog/anki-vs-flashcards-open-source-app/)则详细介绍了成熟度和工作流程方面的取舍。
