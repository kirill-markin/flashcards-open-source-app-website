...
2. 保留原始文件，并核对工作副本；
3. 将映射后的卡片保存到 Nibomo 之前，先完成审核；
4. 将迁移后的 Nibomo 工作区连接到兼容客户端，以便日后读取和写入。

Claude 的连接步骤见[如何通过 MCP 将 Nibomo 连接到 Claude](/blog/how-to-connect-flashcards-to-claude-with-mcp/)。Codex 和其他智能体则可以使用各自运行环境支持的 MCP 或 Agent API 接入方式。

Nibomo 无法读取 Quizlet URL、镜像 Quizlet 账户、同步 Quizlet 后续变更，也无法找回导出时遗漏的图片和学习历史。它会根据你选择迁移的材料创建自己的卡片，并从头建立新的复习排程。
...
Nibomo 将读取操作与写入工具分开：`sql_execute` 用于卡片和牌组，`submit_review` 用于复习。OAuth 凭据仍可调用整套连接器接口，因此能否禁用某项工具、写入前是否需要审批，都由客户端决定。先从一个小型且不含敏感内容的工作区开始，只让智能体读取任务所需的数据。如果客户端支持，让每次写入都单独审批；接受更改之前，先检查工作区和准备提交的修改。
