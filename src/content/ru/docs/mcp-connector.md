...
- `list_workspaces` — строго доступный только для чтения список workspace, к
  ...
  `workspaceId` в качестве аргумента `workspaceId` для `sql_query` и
  `sql_execute`.
...
- **Разделение чтения и записи**: `sql_query` и `list_workspaces` работают строго
  только на чтение (`readOnlyHint`) и не исправляют данные, не пересчитывают
  расписание и не изменяют состояние карточек. `sql_execute` — единственный
  инструмент записи и выполняет запись (`destructiveHint`).
