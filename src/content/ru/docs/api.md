...
- **Разделение чтения и записи**: `sql_query` работает только на чтение (`readOnlyHint`), а `sql_execute` выполняет запись (`destructiveHint`); один вызов должен содержать либо только чтение, либо только запись.
