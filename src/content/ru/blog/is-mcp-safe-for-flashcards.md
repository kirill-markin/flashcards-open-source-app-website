...
20 мая 2026 года Агентство национальной безопасности США (NSA) опубликовало 17-страничное руководство по безопасности Model Context Protocol. Это важно, если колода содержит не только общедоступную лексику: MCP-подключение к Nibomo может передать ИИ-клиенту карточки, метаданные рабочих пространств и историю повторений. Те же учётные данные дают полный доступ к коннектору, включая инструмент, который меняет карточки или помечает их как удалённые. Поэтому вопрос **безопасен ли MCP для карточек** сводится к двум проверкам: можно ли передавать эти данные выбранному клиенту и есть ли у него доступ к инструментам записи.

OAuth защищает процесс авторизации и обмен токенами, а сервер Nibomo ограничивает возможности своих инструментов. Но эти меры не оценивают разумность предложенного изменения, не гарантируют, что полученные данные останутся внутри Nibomo, и не заставляют ИИ-клиент запрашивать подтверждение перед записью.

Смотреть стоит на конкретные границы: к каким данным и действиям открывает доступ каждый инструмент, какие ограничения обеспечивает Nibomo и какие меры защиты работают только в вашем клиенте.

![Тёплый рабочий стол с отдельными секциями для чтения и записи через MCP Nibomo](/blog/is-mcp-safe-for-flashcards.png)
...
4. MCP-сервер и бэкенд Nibomo

Некоторые продукты объединяют клиент и поставщика модели. Другие передают результаты инструментов отдельному сервису. Гарантировано только одно: Nibomo возвращает запрошенные данные аутентифицированному MCP-клиенту. Дальше всё зависит от архитектуры, тарифного плана и настроек клиента. Результат может попасть в контекст модели, остаться в инфраструктуре одного поставщика или перейти к другому обработчику.

На практике риски связаны с чтением, записью и интерпретацией инструкций. При чтении клиент может получить текст карточек, структуру колод, настройки рабочих пространств или события повторений. Запись может создать ненужные карточки, изменить содержимое, пометить карточки и колоды как удалённые или записать повторение, которое изменит расписание карточки. Наконец, агент может неверно понять ваш запрос или принять инструкции из импортированного материала за команды.
...
## Что именно защищает OAuth в Nibomo

Для интерактивных MCP-клиентов Nibomo использует OAuth-поток с кодом авторизации, PKCE и Dynamic Client Registration. Вы подтверждаете подключение в браузере, а PKCE привязывает обмен кодом к клиенту, который его начал. Сервер также проверяет, что токен доступа был выдан именно для MCP-ресурса Nibomo. В [руководстве по MCP-коннектору Nibomo](/ru/docs/mcp-connector/) указаны адрес подключения и метаданные обнаружения.
...
Сейчас Nibomo объявляет одну область доступа OAuth: `flashcards`. Отдельных OAuth-разрешений на чтение и запись нет, а учётные данные дают доступ ко всем инструментам коннектора. Поэтому режим «только чтение» существует только на стороне ИИ-клиента: он должен отключить или заблокировать и `sql_execute`, и `submit_review`. Сервер гарантирует, что сам `sql_query` не умеет записывать данные. Но те же учётные данные позволяют вызвать любой из инструментов записи, если клиент отправит такой запрос.
...
## Что на самом деле умеют MCP-инструменты Nibomo

Коннектор не даёт выполнять произвольные запросы PostgreSQL: встроенный парсер разрешает только запросы из ограниченного SQL-диалекта. У каждого из семи инструментов своя зона доступа:
...
| `get_guide` | Возвращает одно фиксированное справочное руководство по SQL-диалекту, созданию карточек, массовому созданию карточек или процессу повторения и не читает данные рабочего пространства | Нет | Включайте: он возвращает документацию, а не ваши карточки |
| `next_review_card` | Возвращает лицевую сторону следующей карточки к повторению в одном указанном рабочем пространстве | Нет | Включайте для сессии повторения и помните, что текст карточек попадает к клиенту |
| `reveal_answer` | Возвращает обратную сторону одной карточки в одном указанном рабочем пространстве | Нет | Включайте вместе с `next_review_card` |
| `submit_review` | Записывает одну оценку Again, Hard, Good или Easy и сдвигает вперёд расписание FSRS этой карточки | Да | Оставляйте выключенным, если не хотите, чтобы агент записывал повторения за вас |
...
Каждый SQL-вызов работает ровно с одним рабочим пространством. Перед выполнением запроса Nibomo заново проверяет, что у пользователя всё ещё есть к нему доступ. Поэтому вызывающая сторона не может получить доступ к чужому пространству, просто передав его ID.
...
`list_workspaces` и `sql_query` не могут менять состояние карточек. Они также не умеют исправлять данные или пересчитывать расписание. Это разделение обеспечивается сервером и заметно снижает вероятность случайной записи, когда `sql_execute` и `submit_review` недоступны клиенту.

Но возвращённые данные всё равно покидают бэкенд Nibomo. Запрос о темах, которые вы знаете хуже, может включать текст карточек и события повторений. Даже короткая карточка может содержать данные о пациенте, название внутренней системы, личный пример для изучения языка или заметки к собеседованию.

[Политика приватности Nibomo](/ru/privacy/) распространяется на данные, запрошенные через MCP и Agent API. На границе протокола Nibomo передаёт результат MCP-клиенту. Получит ли его отдельный поставщик модели, как долго данные будут храниться и можно ли использовать их для обучения, зависит от архитектуры и настроек клиента, а также от условий поставщика. Проверяйте эти детали: режим «только чтение» ограничивает изменения, но не гарантирует приватность.

### У доступа на запись меньше полномочий, чем у полного доступа к базе данных

`sql_execute` поддерживает `INSERT`, `UPDATE` и `DELETE`, но только для `cards` и `decks`. Ресурсы `workspace` и `review_events` доступны только для чтения. Поля расписания карточек, включая даты следующего повторения, число повторений и сохранённое состояние FSRS, через этот диалект тоже доступны только для чтения. SQL не может зарегистрировать результат учебного повторения или напрямую переписать состояние расписания FSRS. Повторение попадает в расписание только через `submit_review`, а сохранённое повторение нельзя изменить или отменить через MCP.
...
[Условия использования](/ru/terms/) просят пользователей проверять результаты работы ИИ перед применением изменений. Здесь это особенно важно: Nibomo может проверить тип SQL-оператора, ресурс, столбцы и лимит строк. Причину, по которой вы решили внести изменение, сервер проверить не может.

## За подтверждения отвечает клиент

Nibomo помечает `sql_query` и другие инструменты чтения аннотацией `readOnlyHint`, а `sql_execute` и `submit_review` — аннотацией `destructiveHint`. В стабильной [схеме MCP от 25 ноября 2025 года](https://modelcontextprotocol.io/specification/2025-11-25/schema) аннотации инструментов прямо названы подсказками. Они помогают совместимому клиенту выбрать правила подтверждения, но сами ничего не обеспечивают.

Получив корректный аутентифицированный вызов `sql_execute` или `submit_review`, Nibomo выполняет его сразу. Второго экрана подтверждения со стороны Nibomo нет. Одобрение человеком возможно только в ИИ-клиенте, до того как запрос дойдёт до сервера.
...
- Если можно отключать отдельные инструменты, оставляйте `sql_execute` и `submit_review` выключенными, пока они не понадобятся для конкретной задачи.
...
- Если клиент не умеет блокировать инструменты записи или надёжно останавливаться перед вызовами, с самого начала считайте подключение доступом на запись.
...
Удаление облачного аккаунта, отключение коннектора и удаление копий дальше по цепочке — разные операции. [Политика приватности Nibomo](/ru/privacy/) описывает удаление данных из облачной версии. Nibomo не может удалить данные, которые уже сохранил клиент или поставщик модели. Для этого понадобятся средства управления того поставщика.

## Отключение коннектора и отзыв учётных данных — разные действия

Текущая реализация OAuth в Nibomo выдаёт токены доступа на один час и ротируемые refresh-токены без фиксированного срока действия. Сейчас в Nibomo нет пользовательского интерфейса для отзыва OAuth-подключений или публичного API для отзыва OAuth-токенов. При удалении коннектора клиент может удалить свои учётные данные, но это действие на его стороне не гарантирует аннулирование токена на сервере.

Ключи Agent API с префиксом `fca_`, рассчитанные на сценарии без интерактивного входа, — отдельный тип учётных данных. Их можно отозвать в разделе **Agent Connections** в Nibomo. Не смешивайте эти два способа аутентификации, когда описываете или закрываете доступ.
...
2. Проверьте URL сервера. Документированный адрес — `https://mcp.nibomo.com/mcp`. Избегайте доменов-двойников и описаний коннекторов, скопированных из неизвестных источников.
3. Прочитайте политики обеих сторон. Начните с [политики приватности Nibomo](/ru/privacy/), затем проверьте правила конкретного ИИ-клиента о хранении, обучении, памяти, логировании и удалении.
...
5. Сначала заблокируйте `sql_execute` и `submit_review` в клиенте. Если клиент не умеет этого делать, ещё до подключения учтите, что OAuth-учётные данные по-прежнему позволяют выполнять запись.
...
Когда примете эти решения, пошаговую настройку можно найти в статье [Как подключить Nibomo к Claude через MCP](/ru/blog/how-to-connect-flashcards-to-claude-with-mcp/). Руководство объясняет, куда нажимать, а этот чек-лист помогает решить, стоит ли подключать MCP к конкретной колоде.

## Чем помогают открытый исходный код и самостоятельное развёртывание

У коннектора Nibomo есть несколько полезных свойств: отдельные инструменты чтения и записи, закрытый список разрешённых SQL-операторов, проверка доступа к рабочему пространству при каждом вызове, поля расписания, которые нельзя изменить через SQL, и открытый исходный код. Так проще проверить и ограничить доступ коннектора. Эти меры снижают риск, но не могут гарантировать безопасность клиента или правильность решения модели.

[Самостоятельное развёртывание](/ru/docs/self-hosting/) позволяет разместить данные и операции Nibomo в подконтрольной вам инфраструктуре. Но запросы к внешнему ИИ-сервису всё равно выносят данные карточек за её пределы. Цепочка клиента и модели должна отвечать тем же требованиям приватности, что и база данных.

## Простое правило для принятия решения

Используйте инструменты чтения MCP, если допустимо, что запрошенные данные покинут Nibomo и пройдут по выбранной цепочке клиента, условия поставщика вас устраивают, а задача оправдывает такую передачу. Считайте подключение полным доступом, если клиент на самом деле не заблокировал `sql_execute` и `submit_review`.

Включайте инструменты записи только для узкой задачи, когда клиент может остановиться перед каждым важным вызовом, вы заранее проверили целевые строки и у вас есть пригодная резервная копия. Помните: один пакет может затронуть гораздо больше 100 записей, а отменить удаление через MCP нельзя.

Откажитесь от подключения, если колоду нельзя передавать клиенту или его обработчикам, дальнейшие правила работы с данными неясны, на одном аккаунте нужна строгая изоляция рабочих пространств, требуется немедленный отзыв OAuth или сценарий предполагает разрушительные изменения без подтверждения. В таких случаях используйте Nibomo без MCP либо выберите развёртывание и путь к модели, в которых весь поток данных соответствует вашим требованиям.
