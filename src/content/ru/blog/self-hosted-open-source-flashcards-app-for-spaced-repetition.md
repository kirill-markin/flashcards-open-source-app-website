...
Когда у вас приложение с карточками, которое можно развернуть самостоятельно, исходная ситуация меняется. Вы можете изучить код. Можете поднять весь стек у себя. Можете сначала пользоваться размещенной версией, а позже переехать на свою установку. Вам не нужно спрашивать разрешения, чтобы и дальше пользоваться собственной системой обучения так, как удобно именно вам.
...
В [Flashcards](https://flashcards-open-source-app.com/) текущее веб-приложение уже включает чат с ИИ, связанный с реальным рабочим пространством. Более широкая архитектура также открывает отдельную внешнюю поверхность для агентных инструментов в терминале, а клиент для iOS сохраняет собственный офлайн-ориентированный путь синхронизации.
...
Вот что здесь должно означать "AI-first". Не чат-бот, наскоро прикрученный к закрытому приложению, а продукт, где реальные объекты и действия доступны ИИ контролируемым образом.
...
- размещенное веб-приложение, которым можно пользоваться уже сейчас
...
- документированная внешняя агентная API-поверхность
- вход без пароля
...
Проект пока еще на ранней стадии, и я не собираюсь делать вид, что это не так. Но продукт уже давно больше, чем просто прототип, работающий только в браузере: репозиторий включает размещенное веб-приложение, клиент для iOS, сервис аутентификации, серверный API и текущий путь синхронизации. Я скорее буду пользоваться чем-то ранним, но честным, чем чем-то отполированным и запертым.
...
Мы умеем создавать продукты быстрее, чем когда-либо. Можем держать их небольшими. Можем выпускать открытый исходный код. Можем связывать ИИ с реальными действиями внутри продукта, а не только с демонстрационной магией. Можем предлагать размещенную версию, не заставляя пользователя навсегда зависеть от нее.
...
Не потому, что "AI-first" красиво смотрится на лендинге. А потому, что у этой категории наконец-то появились инструменты, чтобы она заслужила более качественный продукт.
