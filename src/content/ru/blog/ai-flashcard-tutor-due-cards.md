...
description: "Подключите Claude, ChatGPT или Codex к Flashcards через MCP. ИИ-репетитор проходит с вами карточки, до которых дошла очередь, проверяет каждый ответ и записывает оценку как повторение по FSRS."
date: "2026-07-15"
updated: "2026-09-16"
...
Поменяйте в промпте лимит на пять и настройте клиент так, чтобы он спрашивал перед запуском `submit_review`, как это позволяют Claude и Codex. Прежде чем подтвердить очередной вызов, сравните оценку, которую репетитор хочет отправить, с той, которую нажали бы сами, и отклоняйте те, с которыми не согласны. Потом откройте [Flashcards](https://app.flashcards-open-source-app.com/) и посмотрите, на какие даты эти повторения перенесли карточки. Если оценки репетитора совпали с вашими, дайте ему подольше поработать самостоятельно. Если нет, вы узнали об этом через пять повторений, а не после недели повторений. ChatGPT может запросить подтверждение в зависимости от разрешений приложения и вашего рабочего пространства, но рассчитывать на это нельзя. Там, как и в любом клиенте без надёжного шага подтверждения, начните с ручных оценок или называйте оценку в каждом ответе.
