...
События описывают, как вы пользуетесь приложениями, а не то, что вы учите: например, какие экраны вы открываете; когда начинается и завершается сеанс повторения, включая его длительность и число отвеченных карточек; успешно или с ошибкой прошли ключевые действия, такие как создание карточки, вход в учётную запись или синхронизация. Каждое событие содержит случайный идентификатор установки, идентификатор сеанса и технический контекст, например платформу, версию приложения, модель устройства, версию операционной системы, локаль, часовой пояс и примерную страну; событие также связывается с рабочим пространством, в котором вы работаете, а если вы вошли в учётную запись — и с ней. Ни одно событие никогда не содержит произвольного текста, содержимого карточек или колод и вашего адреса электронной почты.

Отдельной настройки, отключающей аналитику продукта, нет. При удалении учётной записи идентификаторы в уже собранных событиях заменяются значением, которое нельзя связать с вами, как описано в разделе о сроках хранения и удалении.
...
- События аналитики продукта не удаляются при удалении учётной записи. Мы заменяем их идентификаторы значением, которое нельзя связать с вами, и сохраняем сами отдельные события: после этого их нельзя отнести ни к вам, ни к какому-либо другому человеку.
