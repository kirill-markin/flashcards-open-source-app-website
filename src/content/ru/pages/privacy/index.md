...
Отдельно сайт отправляет события в нашу собственную аналитику продукта: событие просмотра для каждой открытой вами страницы и событие клика, когда вы переходите по ссылке в веб-приложение, в магазин приложений или к установке колоды из каталога. В зависимости от события они содержат тип страницы, публичный идентификатор версии пакета на страницах колод каталога и в ссылках установки, место ссылки, язык интерфейса страницы, язык устройства из браузера, категорию источника перехода, категорию устройства и описанный ниже идентификатор `analytics_visitor`, если браузер его хранит. Этот обработчик не определяет страну по этим событиям. Если браузер сообщает Global Privacy Control или Do Not Track, сайт не отправляет в этот обработчик никаких событий, не запрашивает идентификатор посетителя и не показывает баннер согласия.
