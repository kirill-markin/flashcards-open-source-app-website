...
description: "Finde im ChatGPT Study Mode Wissenslücken, speichere ausgewählte Karten mit Codex und Nibomo MCP in der ChatGPT-Desktop-App und wiederhole sie mit FSRS."
date: "2026-08-02"
...
Wenn du **ChatGPT und Codex zum Lernen nutzen** willst, würde ich es so machen: Lass ChatGPT eine echte Wissenslücke aufdecken, wähle die Karte selbst aus und lass Codex sie über den Nibomo-MCP-Connector speichern. Die Karte landet in deiner Nibomo-Sammlung, wo FSRS sie dir später wieder vorlegt.
...
## Die sinnvolle Aufteilung: ChatGPT erklärt, Codex speichert, Nibomo plant
...
3. **Codex in der ChatGPT-Desktop-App oder in der CLI** erstellt oder überarbeitet die freigegebenen Karten mit Nibomo MCP.
...
Nur der dritte Schritt braucht MCP. Du kannst dir MCP als Brücke vorstellen, über die Codex einige freigegebene Werkzeuge von Nibomo nutzt. Diese Werkzeuge können deine Workspaces auflisten, freigegebene Daten aus Nibomo lesen und nach deiner Zustimmung Karten oder Decks verändern.
...
## Verbinde Codex mit dem Nibomo-MCP-Server

Die Einrichtung in der Desktop-App ist kurz:

1. Öffne **Settings > MCP servers** in der ChatGPT-Desktop-App.
2. Wähle **Add server**.
3. Nenne den Server `nibomo`, wähle **Streamable HTTP** und gib diese URL ein:
...
5. Wähle nach dem Neustart **Authenticate** und schließe die OAuth-Anmeldung bei Nibomo im Browser ab.
6. Gib in Codex `/mcp` ein, um zu prüfen, ob der Server verbunden ist.

Diese Schritte entsprechen OpenAIs aktueller [MCP-Einrichtung für die ChatGPT-Desktop-App und Codex](https://learn.chatgpt.com/docs/extend/mcp). Nibomo nutzt OAuth für die interaktive Anmeldung. In einer normalen Desktop-Sitzung musst du daher keinen API-Key einfügen. Die [Nibomo-MCP-Dokumentation](/de/docs/mcp-connector/) beschreibt die Authentifizierung und alle verfügbaren Werkzeuge, falls die Verbindung nicht funktioniert.
...
```text
Nutze den Nibomo-MCP-Server. Liste zuerst meine Workspaces auf und frage mich,
...
Öffne die [Nibomo-Web-App](https://app.nibomo.com/) oder eine mobile App, sobald die Karten fällig sind. Rufe die Antwort aus dem Gedächtnis ab, decke die Rückseite auf und wähle dort deine Bewertung. Nibomo zeichnet die offizielle Wiederholung auf, danach entscheidet FSRS, wann die Karte wieder fällig wird.
...
Für eine Sitzung ohne grafische Oberfläche, in der die OAuth-Anmeldung im Browser nicht praktikabel ist, akzeptiert Nibomo einen langlebigen `fca_`-Agent-Key als Bearer-Token. Bewahre den Key in einer Umgebungsvariable auf und hinterlege nur deren Namen, nicht den Key selbst:
...
Lies sie einmal durch, kopiere die brauchbaren Karten in Nibomo und fahre mit der normalen Wiederholung fort. Die manuelle Eingabe ist eine vernünftige Wahl für vertrauliches Material, ein verwaltetes Schulkonto oder wenn du nicht möchtest, dass ein KI-Client gespeicherte Kartendaten liest.
...
Mein bevorzugter **Ablauf zum Lernen mit ChatGPT und Codex** ist einfach: Lass dich abfragen, achte darauf, wo du ins Stocken gerätst, gib einige klare Karten frei, speichere sie über Nibomo MCP und erledige die eigentliche FSRS-Wiederholung in Nibomo. Der Connector erspart dir das Kopieren und Einfügen. Du entscheidest weiterhin selbst, was du später noch einmal üben willst.
