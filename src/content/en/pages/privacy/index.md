...
Separately, the website sends events to our own product analytics collector: a page-view event for each page you open; a click event when you follow a link into the web app, an app store, or a catalog deck install; what you search for, filter, sort, page through, and open in the public deck catalog; a click on a link that leads off the site; copying the agent endpoint; the language-suggestion banner being shown and your answer to it; and the cookie decision and the collection switch described below. Depending on the event, they carry the page type, the public package version identifier on catalog deck pages and install links, link placement, which of log in, sign up, or open app a click into the web app was for, which of the banner or the analytics control in the corner a cookie answer was given in, the language the suggestion banner offered, page interface language, browser device language, source category, device category, and the `analytics_visitor` identifier described below when your browser carries one. A page-view event also carries the host of the site you came from, reported from a list of known search and social sites and as `other` for anything else, and the `utm_source`, `utm_medium`, and `utm_campaign` parameters of the address you arrived on. A catalog search, filter, sort, or page change carries how many results you saw and what you chose: the filter action — adding, selecting, removing, or clearing — with its category and how many filters are selected afterwards, the sort order, or the page number together with the total number of pages. Opening a deck from the catalog carries the deck's public identifier and where on the page you opened it from, and no result count. A catalog search also carries the length of what you typed and, only when the normalized text is at most 64 characters long and contains nothing but letters, the marks that combine with them, digits, spaces, and hyphens, that text itself; any other query is reported by its length alone and is never shortened to fit. Our server derives a two-letter country code from the address each event arrives from and stores it with the event; it stores no address, and it derives no country for the events that carry no identifier at all, described below. When your browser exposes Global Privacy Control, the website requests no visitor identifier and shows no consent banner, while the events above still reach our own collector without that identifier, and all of them except the events that carry no identifier at all receive the daily identifier described below. We do not read the Do Not Track setting.
...
The marketing website and the hosted web app share one first-party analytics cookie, `analytics_visitor`. It holds a random visitor identifier, can be read by our own scripts in the browser, and expires after 13 months. We use it to recognize the same browser across visits, so events show a returning visitor instead of a new one every time. It is not strictly necessary. This decision covers only whether your browser carries that identifier; whether the website collects anything at all is a separate switch, described below. Where consent is required, including the EU/EEA and the UK, we set it only after you agree: the website asks with a banner, and until you answer it requests no identifier and attaches none to anything it sends, while the events described above are still sent without it, and all of them except the events that carry no identifier at all receive the daily identifier described in the next paragraph. Where consent is not required, we set it without asking. Either way you can turn it off at any time, from the analytics control in the corner of every website page or in the web app settings; we then clear the cookie from your browser and stop using it. Your answer is stored in your browser so you are not asked again. When your browser exposes Global Privacy Control, we clear that cookie, and with it an answer that had allowed it, so that browser is treated as a new one if the signal is later switched off: asked with the banner where consent is required, and, where consent is not required, given the cookie without asking. An answer that declined the cookie is kept, and it still applies. We also record this cookie decision as events: that the banner was shown, and what you answered, wherever you answered it — in the banner, or in the analytics control in the corner that holds the same choice. The recorded answer carries which of those two places it was given in. The event for the banner being shown and the event for a declined answer carry no identifier at all; the event for an agreed answer carries the identifier you have just allowed. The events themselves are kept while we need them for the purposes described in this policy.

When an event reaches our collector without the `analytics_visitor` identifier, for example because you declined the cookie, have not answered yet, or turned it off, our server derives a temporary identifier from the request's IP address and browser user agent, combined with a random value that changes every UTC day and is deleted once that day ends. The IP address itself is not stored in our analytics. This identifier only links that day's events from the same browser and IP address. It is never stored in your browser, never linked to the `analytics_visitor` identifier or to an account, never attached to a record of your consent answer, and cannot be recomputed once that day's random value is deleted. We use it to understand how visitors use the website, based on our legitimate interest described in the Purposes and Legal Bases section. A browser exposing Global Privacy Control never carries the `analytics_visitor` identifier, so its events receive this daily identifier like any other browser without that cookie. The events that carry no identifier at all receive neither this daily identifier nor a country: the consent banner being shown, a declined answer, and turning this website's collection off or back on. They carry no `analytics_visitor` identifier, no daily identifier, and no country — only what happened and, for a cookie answer, where you gave it.

The analytics control in the corner of every website page holds the switch that turns this website's collection off, and alongside it the analytics cookie consent described above; once you have answered the banner, the control also holds that cookie choice while collection is on, and offers it again when you turn collection back on. When your browser exposes Global Privacy Control, the control holds only that switch: we give that browser no analytics cookie at all, so there is no cookie choice to offer. Turning it off sends one last event recording only that it was turned off, and turning it back on sends the matching one; both carry no identifier, no daily identifier, and no country. While collection is off, the website sends no other event about your visit, neither to our collector nor to Vercel Web Analytics, and it asks for no visitor identifier; because nothing else reaches our collector, no daily identifier is derived either. It is on unless you turn it off, on the legitimate interest described in the Purposes and Legal Bases section. This website has no accounts, so your answer is stored in that browser and acts only there; the hosted web app has its own setting, and the iOS and Android apps have it from the app version that introduces it, both described in the Product Analytics section.
...
Events describe screens, review-session duration and counts, and whether actions succeed or fail. They can include installation and session identifiers and, where applicable, workspace and account identifiers. These app events do not include free text, card or deck content, or your email address.
