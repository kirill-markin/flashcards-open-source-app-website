...
AIチューターでも、同じ境界を守る必要があります。[AIフラッシュカードチューターのワークフロー](/ja/blog/ai-flashcard-tutor-due-cards/)では、チューターが答えを見て知った内容ではなく最初の回答を採点し、その評価を`submit_review`で正式なFSRSの復習として記録します。採点のルールは`get_guide`のトピック`review_flow`で提供され、記事で紹介しているチューター用プロンプトには、ヒントが必要だった回答はAgainにするという、このセクションと同じルールが加えてあります。それでも評価はモデルの判断なので、最終的に自分で決めたいなら、答えと一緒に評価を伝えるか、手動評価を頼んでください。
