...
これは Nibomo でも同じです。このアプリは FSRS の復習で `Again`、`Hard`、`Good`、`Easy` の4段階評価を使います。`Again` が続くのは恥ではありません。データです。そのカードに編集が必要だというサインです。
...
## Nibomoで何度も失敗するカードを扱う流れ

Nibomo を使っているなら、流れはかなりシンプルです。
