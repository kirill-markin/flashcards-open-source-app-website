...
## Quizlet APIの代替としてNibomoが提供するもの

Nibomoは、ユーザーごとに範囲が限られた同じデータ領域へアクセスする方法を二つ公開しています。

- [外部Agent API](/docs/api/)は`GET https://api.nibomo.com/v1/`から始まります。ディスカバリー応答が、メールOTPでのログイン、APIキーの作成、ワークスペースの選択までエージェントを案内します。読み取りにはSQL形式のクエリルート、書き込みには別の実行ルートを使います。
