---
title: セルフホスティングガイド
description: PostgreSQL、認証、バックエンド、Web、管理の各サービスを使ってNibomoをローカルで動かす方法と、文書化されたAWS CDK本番スタックをデプロイする方法を説明します。
---

Nibomoでは、ローカル開発環境とAWS上の本番環境という2つの異なる運用方法をサポートしています。Docker Composeは、ローカル開発用のPostgreSQLとマイグレーションを実行するためのものであり、本番環境のデプロイ方法ではありません。
