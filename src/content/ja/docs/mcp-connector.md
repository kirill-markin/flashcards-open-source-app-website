...
- `list_workspaces` — アクセスできるワークスペースの一覧を厳密な読み取り専用で返します。各ワークスペースには `workspaceId`、名前、アクティブなカード数、最終アクティビティ、そして現在選択中のデフォルトかどうかが含まれます。返された `workspaceId` を `sql_query` と `sql_execute` の `workspaceId` 引数に使います。
...
- **読み取り／書き込みの分離**: `sql_query` と `list_workspaces` は厳密な読み取り専用（`readOnlyHint`）であり、データを修復したり、スケジュールを再計算したり、カード状態を変更したりすることはありません。`sql_execute` は唯一の書き込みツールで、書き込みを行います（`destructiveHint`）。
