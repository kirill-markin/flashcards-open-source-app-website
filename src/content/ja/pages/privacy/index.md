...
マーケティングサイトとホスト型ウェブアプリは、`analytics_visitor` という自社の解析 Cookie を1つ共有しています。ランダムな訪問者識別子を保持し、ブラウザ内で当社のスクリプトから読み取れ、13か月で失効します。これは同じブラウザを訪問をまたいで識別し、イベントが毎回新規ではなく再訪問の訪問者として表れるようにするためのものです。この Cookie は厳密に必要なものではありません。この選択が決めるのは、お使いのブラウザがその識別子を保持するかどうかだけです。サイトが何かを収集するかどうかは、以下で説明する別のスイッチで決まります。同意が必要な地域、たとえば EU/EEA と英国では、ご同意をいただいた後にのみ設定します。サイトはバナーでお尋ねし、お答えいただくまでは識別子を要求せず、付与もしません。上記のイベントは、その間もこの識別子を付けずに送信されます。そのうち、識別子をまったく持たないイベントを除くすべてに、次の段落で説明する日ごとの識別子が付与されます。同意が不要な地域では、お尋ねせずに設定します。いずれの場合も、サイトの各ページの隅にある解析コントロール、またはウェブアプリの設定から、いつでも無効にできます。その後、当社は Cookie をブラウザから削除し、使用を停止します。お客様の回答はブラウザに保存され、再度お尋ねすることはありません。バナー自体もイベントとして記録します。表示されたこと、そしてどのように回答されたかです。バナーが表示されたイベントと拒否の回答のイベントは、識別子をまったく持ちません。同意の回答のイベントには、その時点で許可された識別子が付与されます。イベント自体は、本ポリシーに記載した目的のために必要な間だけ保管します。
