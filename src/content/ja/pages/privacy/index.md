...
これとは別に、カタログのインストールリンクは当社のプロダクト解析にクリックイベントを送信します。内容は、公開パッケージのバージョン識別子、ページの表示言語、ブラウザが示す端末の言語、リンク位置、流入元の分類、端末の分類、そしてお使いのブラウザが保持している場合は後述の `analytics_visitor` 識別子です。この収集処理はクリックから国を推定しません。ブラウザが Global Privacy Control または Do Not Track を通知する場合、サイトはこの収集処理にクリックイベントを送信せず、訪問者識別子も要求せず、同意バナーも表示しません。
...
マーケティングサイトとホスト型ウェブアプリは、`analytics_visitor` という自社の解析 Cookie を1つ共有しています。ランダムな訪問者識別子を保持し、ブラウザ内で当社のスクリプトから読み取れ、13か月で失効します。これは同じブラウザを訪問をまたいで識別し、イベントが毎回新規ではなく再訪問の訪問者として表れるようにするためのものです。この Cookie は厳密に必要なものではありません。同意が必要な地域、たとえば EU/EEA と英国では、ご同意をいただいた後にのみ設定します。サイトはバナーでお尋ねし、お答えいただくまでは識別子を要求せず、付与もしません。上記のカタログのインストールリンクのクリックイベントは、その間も識別子を付けずに送信されます。同意が不要な地域では、お尋ねせずに設定します。いずれの場合も、サイトの各ページの隅にある解析コントロール、またはウェブアプリの設定から、いつでも無効にできます。その後、当社は Cookie をブラウザから削除し、使用を停止します。お客様の回答はブラウザに保存され、再度お尋ねすることはありません。イベント自体は、本ポリシーに記載した目的のために必要な間だけ保管します。
