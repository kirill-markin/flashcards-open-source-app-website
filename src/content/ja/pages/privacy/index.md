...
上記の解析 Cookie への同意を撤回すると訪問者識別子の使用は停止しますが、イベント自体は引き続き収集され、サインインしている間はお客様のアカウントに紐づいたままです。アプリ内のプロダクト利用イベントを無効にする個別の設定はありません。アカウントを削除すると、関連するインストールのプロファイルを削除し、保持するイベントの識別子と情報を「保存期間と削除」の項に記載のとおり変更します。
