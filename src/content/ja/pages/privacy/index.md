...
これとは別に、カタログのインストールリンクは当社のプロダクト解析にクリックイベントを送信します。内容は、公開パッケージのバージョン識別子、ページの表示言語、ブラウザが示す端末の言語、リンク位置、流入元の分類、端末の分類です。この収集処理はクリックから国を推定しません。ブラウザが Global Privacy Control または Do Not Track を通知する場合、これらのサイトのクリックイベントは送信されません。
...
ホスト型ウェブアプリは、プロダクトのドメインに `analytics_visitor` という自社の解析 Cookie も設定します。ランダムな訪問者識別子を保持し、ブラウザ内で当社のスクリプトから読み取れ、13か月で失効します。これは同じブラウザを訪問をまたいで識別し、プロダクト利用イベントが毎回新規ではなく再訪問の訪問者として表れるようにするためのものです。この Cookie は厳密に必要なものではありません。同意が必要な地域、たとえば EU/EEA と英国では、ご同意をいただいた後にのみ設定し、後からウェブアプリの設定で同意を撤回できます。撤回後、この Cookie は使用されません。イベント自体は、本ポリシーに記載した目的のために必要な間だけ保管します。
