...
イベントが示すのは、お客様の学習内容ではなくアプリの使われ方です。たとえば、開いた画面、復習セッションの開始と終了（所要時間と回答したカード数を含む）、およびカードの作成、ログイン、同期といった主要な操作が成功したか失敗したかです。各イベントには、インストールごとのランダムな識別子、セッション識別子、およびプラットフォーム、アプリのバージョン、端末モデル、OS のバージョン、ロケール、タイムゾーン、おおよその国といった技術的な情報が含まれます。イベントは、お客様が作業しているワークスペースにも紐づき、ログイン中の場合はお客様のアカウントにも紐づきます。いずれのイベントも自由記述のテキスト、カードやデッキの内容、メールアドレスを含むことはありません。

プロダクト解析を無効にする個別の設定はありません。アカウントを削除すると、すでに収集されたイベントの識別子は、お客様に結び付けられない値に置き換えられます。詳細は「保存期間と削除」の項をご覧ください。
...
- プロダクト解析のイベントは、アカウントを削除しても削除されません。識別子をお客様に結び付けられない値に置き換えたうえで、個々のイベントを保持しますが、以後はお客様にも他のどなたにも結び付けることはできません。
